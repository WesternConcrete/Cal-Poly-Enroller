...
import { ArrowLeft, Check, Copy, Expand, GraduationCap } from "lucide-react";
      ...
      <section className="container flex flex-col items-center justify-start gap-3 pb-10 pt-6 w-[80%] overflow-hidden">
        <div className="flex items-center justify-start w-full">
          ...
          >
            ...
            <CardTitle>View available courses below</CardTitle>
          </Button>
        </div>
        <Accordion type="multiple" className="w-full grid gap-4 overflow-auto">
          {selectedRequirementObjects.map((requirement, index) => (
            <AccordionItem
              value={`item-${index}`}
              key={index}
              className="border-[1px] border-solid rounded"
            >
              <AccordionTrigger className="border px-4 py-3 text-sm w-full text-left rounded">
                <div>{requirement.code}</div>
                {requirement.title}
              </AccordionTrigger>
                      ...
                      <TableRow key={index}>
                          ...
                          <div>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="start">
                                <DropdownMenuItem
                                  onClick={() =>
                                    openRateMyProfessor(section.instructor)
                                  }
                                >
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    openPolyRatings(section.instructor)
                                  }
                                >
                        ...
                        <TableCell className="text-right">
                          <Button
                            variant="link"
                            className="text-left text-blue-600 hover:underline"
                            onClick={() => copyCourse(section.courseCode)}
                          >
                            {section.courseCode}
