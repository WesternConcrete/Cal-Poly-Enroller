...
import { Check, PlusCircle, ChevronsUpDown, CheckCheckIcon, CheckCircle } from "lucide-react"
...
export default function CourseCard({ requirement, index, collapsed }: Props) {
  const { dragging, draggingItem, draggingOver } = React.useContext(DraggingState);
  ...
  const toggleRequirement = (requirement: Requirement) => {
    ...
    if (selectedRequirements.includes(requirement.id)) {
      setSelectedRequirements(selectedRequirements.filter((req) => req !== requirement.id));
    } else {
      ...
      {(provided: DraggableProvided) => {
        return (
          ...
          >
            ...
            p-[.4rem] 
            w-full 
            ${(getCollapsed()) ? 'min-h-[30px] opacity-40 w-[195px]' : 'min-h-[70px] w-[107px]'} 
            ...
            rounded-[0.5rem] 
            hover:border-[2px]
            ${selectedRequirements.includes(requirement.id) && !getCollapsed()? 'border-[2px] border-primaryGreen ':''}
  ...
  <div className="grid justify-items-center grid-rows-[min-content,1fr,min-content] h-full row-gap-[0.2rem] w-full text-center justify-center">
               <div>
                 ...
                 <div className="font-normal text-[9px]">
                  ...
                  <RequirementSelector requirements={available_ge_options} setSelectedRequirement={(req) => setSelectedRequirement(req)} selectedRequirement={selectedRequirement as Requirement}/>
...
<div className="grid justify-items-center grid-rows-[min-content,1fr,min-content] h-full row-gap-[0.2rem] w-full text-center justify-center">
               <div>
                 ...
                 <RequirementSelector requirements={available_ge_options} setSelectedRequirement={(req) => setSelectedRequirement(req)} selectedRequirement={selectedRequirement as unknown as Requirement}/>
               </div>
             </div>
}
             ...
             : 
              ...
              <div>
                ...
                <div
              className={`overflow-hidden max-h-[100px]`}
            >
                  <div className="font-normal text-[9px]">
                    {requirement.title}
                  </div>
...
export function RequirementSelector({ requirements, setSelectedRequirement, selectedRequirement, ...props }: RequirementSelectorProps) {
  const [open, setOpen] = React.useState(false)
  const [peekedRequirement, setPeekedRequirement] = React.useState<Requirement | null>(null)
    ...
    <div className="grid gap-2">
      ...
      <Popover open={open} onOpenChange={setOpen} {...props}>
        <PopoverTrigger asChild>
          {selectedRequirement? 
          <Button variant="ghost" size="sm" className="h-4 border-solid bg-white mt-[.6rem]">
           <ChevronsUpDown className="h-4 w-4 shrink-0 opacity-50"/>
        </Button>
          :
          <Button variant="outline" size="sm" className="h-8 border-solid bg-white mt-[.6rem]">
...
function RequirementItem({ requirement, isSelected, onSelect, onPeek, peekedRequirement }: RequirementItemProps) {
  ...
  <div>
    <HoverCard>
      ...
      {peekedRequirement?.code === requirement.code &&
        ...
        >
          <div className="grid gap-2">
            <h4 className="font-medium leading-none">{peekedRequirement.code}</h4>
