...
import { FlowchartState, useMoveRequirement, DraggingState } from "~/dashboard/state";
import { api } from "~/utils/api";
import CollapsedQuarter from "./CollapsedQuarter";


export default function Flowchart() {
  const { startYear, selectedRequirements, setSelectedRequirements } = React.useContext(FlowchartState);
  ...
  const handleDragEnd = ({
    ...
    if(selectedRequirements.includes(parseInt(draggableId)) && destination && destination.droppableId === "-1") {
      setSelectedRequirements(selectedRequirements.filter((req) => req !== parseInt(draggableId)))
    }
    setDragging(false);
  ...
  return (
    <div className="h-full flex overflow-x-auto">
      <DragDropContext onDragEnd={handleDragEnd} onDragStart={handleDragStart} onDragUpdate={handleDragUpdate}>
        <DraggingState.Provider value={{dragging, setDragging, draggingItem, setDraggingItem, draggingOver, setDraggingOver}}>
