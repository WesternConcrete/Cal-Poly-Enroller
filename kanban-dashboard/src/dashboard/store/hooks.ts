...
export function useUpdateUser() {
  const dispatch = useDispatch();
  return useCallback(
    (id: string, user: { username?: string }) => {
...
export function useMoveStatusCourse() {
  ...
  return useCallback(
    (
      ...
      dispatch(
        actions.moveStatusCourse(taskId, srcStatusId, src, destStatusId, dest)
      );
    },
    [dispatch]
  );
...
export function useCommentUsername(id: string) {
  return useSelector((state: State) =>
    selectors.getCommentUsername(state, { id })
  );
}
